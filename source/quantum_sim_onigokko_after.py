...
TARGET_MARKER = 'ro'
HUNTER_MARKER = 'bx'
SIMULATION_TIME_LIMIT = 2400 # 制限時間 (必要に応じて調整)
...
def get_xy_from_index(index, grid_size):
...
# --- 様々な位置にするqubo ---
def getVariousPositionQubo(target_pos) :
    key=str(target_pos[0]) + KEY_SEPARATOR + str(target_pos[1])
    file_name = "qubo_dic" + key + ".pkl"
    qubo_dictionary = load_dic(file_name)
    vpqubo = qubo_dictionary[key]
    return vpqubo

# --- QUBO構築関数 (v4 - 包囲目的) (内部ロジックは grid_size を使うので変更不要) ---
def build_qubo(target_pos, grid_size, num_hunters, A, A_far,B,C, P1, P2, P3, P4, hunter_positions_list):

    def innerThread0(h1,h2,b_times_dict,grid_size) :
        for dict_key in b_times_dict.keys() :
            memory_idx1,memory_idx2 = dict_key
            x1,y1 = get_xy_from_index(memory_idx1, grid_size)
            x2,y2 = get_xy_from_index(memory_idx2, grid_size)
            idx1 = get_variable_index(h1, x1, y1, grid_size)
            idx2 = get_variable_index(h2, x2, y2, grid_size)
            Q[(idx1, idx2)] += b_times_dict[(memory_idx1, memory_idx2)]
            Q[(idx2, idx1)] += b_times_dict[(memory_idx1 ,memory_idx2)]

    def innerThread1(h,x,y,target_pos,A,A_far,P3,grid_size) :
    ...
    b_times_dict = {key: value * PARAM_B for key, value in dict.items()}
    Q = defaultdict(float)
    threads = []

    for h1 in range(num_hunters-1):
        for h2 in range(h1+1,num_hunters):
            t = threading.Thread(target=innerThread0, args=(h1,h2,b_times_dict,grid_size))
            threads.append(t)
            t.start()
    ...
    for h in range(num_hunters):
        for y in range(grid_size): # ループ範囲が grid_size に依存
            for x in range(grid_size): # ループ範囲が grid_size に依存
                t = threading.Thread(target=innerThread1, args=(h,x,y,target_pos,A,A_far,P3,grid_size))
                threads.append(t)
                t.start()
    for t in threads :
        t.join()
...
if __name__ == "__main__":
    ...
    while elapsed_time < SIMULATION_TIME_LIMIT and not caught:
        ...
        qubo = build_qubo(target_position, GRID_SIZE, NUM_HUNTERS, PARAM_A, PARAM_A_FAR, PARAM_B, PARAM_C, PARAM_P1, PARAM_P2, PARAM_P3, PARAM_P4,hunter_positions_list)
        ...
        if best_sample:
            ...
            if valid_solution_in_step:
                ...
                print("QAの解が無効なため、ハンターは移動しません。")
                print(len(assigned_hunters_in_step))
        else:
            print("QAソルバーから解が得られませんでした。ハンターは移動しません。")
